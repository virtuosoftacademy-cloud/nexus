    ...
    return (
        <section className="py-14 md:py-20 px-4 flex justify-center flex-col items-center">
            ...
            <div className="pb-6 text-center">
                ...
                <h3 className="mt-3 font-heading text-foreground font-semibold text-lg">
                    {subtitle}
                </h3>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-center">
                {items?.map((item) =>
                    <Card key={item.id} className="group rounded-none border-2 border-primary w-full max-w-[32em] h-20 px-18 py-10 bg-primary/5 hover:bg-primary transition-all duration-400 text-center flex justify-center">
                        <CardTitle className="font-heading text-primary text-base md:text-lg group-hover:text-accent whitespace-break-spaces">
                            {item.text}
                        </CardTitle>
                    </Card>
                )
            ...
            <p className="text-center mt-6 font-heading text-foreground">
                {closing}
            </p>
        </section>
    )
