 ...
 * Localization removed: all user-facing text is plain English inline.
 */
    ...
    return (
        <div className={`space-y-6 ${className}`}>
            ...
            <div className="relative">
                <div
                    className={`border-2 border-dashed rounded-xl text-center transition-all duration-200 cursor-pointer ${compact ? "p-4" : "p-8"
                        } ${dragActive
                ...
                >
                    {uploading ? (
                        <div className={compact ? "space-y-2" : "space-y-4"}>
                            <div className="relative">
                                <div
                                    className={`mx-auto bg-blue-100 dark:bg-blue-950/30 rounded-full flex items-center justify-center ${compact ? "w-10 h-10" : "w-16 h-16"
                                        }`}
                                >
                                    <Loader2
                                        className={`text-primary dark:text-primary-400 animate-spin ${compact ? "h-5 w-5" : "h-8 w-8"
                                            }`}
                                    />
                                </div>
                            </div>
                        ...
                        <div className={compact ? "space-y-2" : "space-y-4"}>
                            <div className="relative">
                                <div
                                    className={`mx-auto bg-linear-to-br bg-primary/5 dark:bg-background border-white dark:border-primary rounded-full flex items-center justify-center ${compact ? "w-10 h-10" : "w-16 h-16"
                                        }`}
                                >
                                    <Upload
                                        className={`text-primary dark:text-primary-400 ${compact ? "h-5 w-5" : "h-8 w-8"
                                            }`}
                                    />
                                </div>
                                {dragActive && (
                                    <div
                                        className={`absolute inset-0 mx-auto bg-primary-200 dark:bg-primary rounded-full animate-ping opacity-75 ${compact ? "w-10 h-10" : "w-16 h-16"
                                            }`}
                                    ></div>
                                )}
                            </div>
                            <div className={compact ? "space-y-1" : "space-y-2"}>
                                ...
                                {!compact && (
                                    <p className="text-sm text-gray-600 dark:text-gray-400">
                                        {disabled
                                            ? "Image uploading is currently unavailable"
                                            : "or click anywhere in this area to browse your files"}
                                    </p>
                                )}
                            </div>
                            <div
                                className={`flex items-center justify-center text-xs text-gray-500 dark:text-gray-400 ${compact ? "space-x-3" : "space-x-4"
                                    }`}
                            >
            ...
            {images.length > 0 && (
                <div className="mt-6 space-y-4">
                    ...
                    >
                        {images.map((image, index) => (
                            <div key={image.publicId || index} className="relative group">
                                <div
                                    className={`relative overflow-hidden rounded-lg border-2 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 hover:border-primary-300 dark:hover:border-primary transition-colors ${compact ? "aspect-4/3 h-20" : "aspect-4/3 h-32"
                                        }`}
                                >
                                    <Image
                                        src={image.url}
                                    ...
                                    <div className={`absolute ${compact ? "top-1 left-1" : "top-2 left-2"}`}>
                                        <div
                                            className={`bg-green-500 rounded-full flex items-center justify-center shadow-lg ${compact ? "w-4 h-4" : "w-6 h-6"
                                                }`}
                                        >
                    ...
                    {/* Upload Summary */}
                    <div
                        className={`bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-lg ${compact ? "p-2" : "p-3"
                            }`}
                    >
                        <div
                            className={`flex items-center text-green-800 dark:text-green-400 ${compact ? "text-xs" : "text-sm"
                                }`}
                        >
                            <CheckCircle className={`mr-2 text-green-600 ${compact ? "h-3 w-3" : "h-4 w-4"}`} />
                            <span className="font-medium">
                                {images.length} image{images.length > 1 ? "s" : ""} ready to use
                            </span>
                        </div>
                        <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                            Review your images above — remove any you don&apos;t want before saving.
                        </p>
                    </div>
                </div>
            )}
