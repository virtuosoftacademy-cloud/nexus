 ...
 * Purpose: Client-side utilities for R2 integration on the Nexus site —
 *          validation, URL helpers, object-key generation. Adapted from the
 *          PropertyPro version for this project's needs:
 *            - One image per entity (post cover / case-study hero),
 *              so no gallery or multi-file concerns
 *            - Folders: "posts" and "case-studies"
 *            - Branding/favicon section replaced with cover/hero validation
 *              matching how the images render (16:9 cards, full-bleed hero)
 *          Server-side upload lives in lib/r2.ts (S3 API) — never import
 *          that from client code.
 * ============================================================================
 */
...
// ── Nexus-specific: cover/hero validation ──────────────────────────
// Post covers render 16:9 on blog cards (1200x675 target) and inline on
// the post page. Case-study heroes render full-bleed as a CSS background
// (no next/image optimization), so width matters most and oversized
// originals cost real bandwidth.
